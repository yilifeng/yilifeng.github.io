...
<template>
  <div class='calendar'>
    ...
    <table cellpadding='5'>
      ...
      <tbody>
      <tr v-for='(day, k1) in days' :key="day" style="{'animation-delay',(k1*30)+'ms'}">
        <td v-for='(child, k2) in day' :key="child" :class="{'selected':child.selected,'disabled':child.disabled}" @click='select(k1, k2, $event)'>
    ...
    <div class='calendar-years' :class="{'show':yearsShow}">
      <span v-for="y in years" @click.stop="selectYear(y)" :key="y" :class="{'active':y==year}">{{y}}</span>
    </div>
  </div>
</template>
